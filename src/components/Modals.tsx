...
import { Fuel, X, Zap, Droplet, ArrowDownLeft, ArrowUpRight, Crown, Mail, MessageCircle, HelpCircle, Check, ShieldCheck, ChevronRight } from 'lucide-react';
...
export const ExpenseModal: React.FC<ExpenseModalProps> = ({ onClose, onSave, editItem, currentOdometer }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-white w-full max-w-md rounded-3xl p-6 shadow-2xl animate-in slide-in-from-bottom-8 max-h-[90vh] overflow-y-auto">
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <div className="relative mt-1.5">
                                <select name="category" value={cat} onChange={e => setCat(e.target.value)} className="w-full p-4 bg-slate-50 rounded-2xl border border-slate-100 outline-none text-slate-700 font-bold appearance-none">
                        ...
                        {cat === 'Fuel' && (
                            <div className="bg-blue-50/50 p-5 rounded-2xl border border-blue-100 space-y-4">
                                ...
                                {!editItem && (
                                    <div>
                                        <label className="text-xs text-blue-600 font-bold block mb-1.5">Current Odometer (Optional)</label>
                                        <div className="flex items-center gap-2 bg-white p-1 rounded-xl border border-blue-100 shadow-sm">
                                            <input name="linkedOdometer" type="number" placeholder={currentOdometer.toString()} className="w-full p-2 rounded-lg outline-none font-bold text-slate-700 pl-3" />
                                            <span className="text-xs text-blue-400 font-bold pr-3">km</span>
                                        </div>
                                    </div>
                                )}
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input name="fuelPrice" type="number" step="0.01" placeholder="e.g. 102" value={price} onChange={e => setPrice(e.target.value)} className="w-full p-3 bg-white rounded-xl border border-blue-100 outline-none font-bold text-slate-700 shadow-sm" />
                                    </div>
                                    <div>
                                        <label className="text-xs text-blue-600 font-bold block mb-1.5">Liters</label>
                                        <input name="fuelVolume" type="number" step="0.01" placeholder="Auto-calc" value={vol} onChange={e => setVol(e.target.value)} className="w-full p-3 bg-blue-100/50 rounded-xl border border-blue-100 outline-none font-bold text-blue-800" />
                                    </div>
                                </div>
                            </div>
                        )}
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div><label className="text-sm font-bold text-slate-700 ml-1">Date</label><input name="date" type="date" required defaultValue={editItem?.date || new Date().toISOString().split('T')[0]} className="w-full p-3.5 mt-1.5 bg-slate-50 rounded-2xl border border-slate-100 outline-none font-medium text-slate-600" /></div>
                            <div><label className="text-sm font-bold text-slate-700 ml-1">Note</label><input name="note" placeholder="Short note..." defaultValue={editItem?.note} className="w-full p-3.5 mt-1.5 bg-slate-50 rounded-2xl border border-slate-100 outline-none font-medium" /></div>
                        </div>
                    </div>
        ...
        <div className="bg-white w-full max-w-md rounded-3xl p-6 shadow-2xl animate-in slide-in-from-bottom-8">
            ...
            <form onSubmit={onSave} className="space-y-6">
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                    ...
                    <div className="flex items-center"><input name="reading" type="number" step="0.1" required defaultValue={editItem?.odometer || (currentOdometer ? currentOdometer + 10 : '')} placeholder="0" className="w-full text-3xl font-black bg-transparent outline-none text-slate-800" /></div>
                    {!editItem && <p className="text-xs text-slate-400 mt-2 font-medium flex items-center gap-1"><Check className="w-3 h-3" /> Last recorded: {currentOdometer} km</p>}
                </div>
                <div>
                    <label className="text-sm font-bold text-slate-700 ml-1">Fuel Status</label>
                    <div className="flex gap-3 mt-2">
                        <label className="flex-1 cursor-pointer group">
                            ...
                            <div className="p-3.5 text-center rounded-2xl border-2 border-slate-100 text-slate-400 peer-checked:bg-blue-600 peer-checked:text-white peer-checked:border-blue-600 peer-checked:shadow-lg peer-checked:shadow-blue-200 transition-all font-bold text-sm flex items-center justify-center gap-2 group-hover:border-slate-200"><Zap className="w-4 h-4" /> Main Tank</div>
                        </label>
                        <label className="flex-1 cursor-pointer group">
                            <input type="radio" name="fuelStatus" value="Reserve" defaultChecked={editItem?.fuelStatus === 'Reserve'} className="peer sr-only" />
                            <div className="p-3.5 text-center rounded-2xl border-2 border-slate-100 text-slate-400 peer-checked:bg-orange-500 peer-checked:text-white peer-checked:border-orange-500 peer-checked:shadow-lg peer-checked:shadow-orange-200 transition-all font-bold text-sm flex items-center justify-center gap-2 group-hover:border-slate-200"><Droplet className="w-4 h-4" /> Reserve</div>
                        </label>
                    </div>
                </div>
                <div><label className="text-sm font-bold text-slate-700 ml-1">Date</label><input name="date" type="date" required defaultValue={editItem?.date || new Date().toISOString().split('T')[0]} className="w-full p-3.5 mt-1.5 bg-slate-50 rounded-2xl border border-slate-100 outline-none font-medium text-slate-600" /></div>
...
// --- POLISHED SETTINGS MODAL WITH TRUST UI ---
