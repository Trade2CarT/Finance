            ...
            <div className="bg-white w-full max-w-md rounded-2xl p-8 shadow-xl border border-slate-100">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2 bg-slate-50 p-3 rounded-xl border border-slate-200 focus-within:border-orange-500 focus-within:ring-2 focus-within:ring-orange-200 transition-all">
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="w-full bg-slate-800 text-white font-bold py-3.5 rounded-xl hover:bg-slate-900 shadow-lg shadow-slate-200 flex justify-center items-center gap-2 transition-all active:scale-95 disabled:opacity-70"
                    >
                        {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : (
                            <>
                                {isReset ? 'Send Reset Link' : (isLogin ? 'Sign In' : 'Create Account')}
                                {!isReset && <ArrowRight className="w-4 h-4" />}
                            </>
                        )}
                    </button>
                </form>
                ...
                <div className="mt-6 space-y-3 text-center">
                    {!isReset ? (
                        <>
                            <button onClick={() => setIsReset(true)} className="text-sm text-slate-400 hover:text-orange-600 font-medium">
                                Forgot Password?
                            </button>
                        ...
                        <button onClick={() => setIsReset(false)} className="text-sm text-slate-500 hover:text-slate-800 font-bold">
                            Back to Login
                        </button>
                    )}
                </div>
